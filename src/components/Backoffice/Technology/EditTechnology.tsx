...
const EditTechnology = () => {
  ...
  let [logo, setLogo] = useState(state.logo);
  let [description, setDescription] = useState(state.description);
  ...
  const handleSubmit = (event: {
    ...
    if (form.checkValidity() === false) {
      ...
      saveTechnology(name, logo, description);
    }

    setValidated(true);
  };

  const saveTechnology = (name: string, logo: string, description: string) => {
    ...
    fetch(`api/technology/${state.id}`, options)
      .then((response) => {
          ...
          <>
            <Button>
              <Nav.Item className="nav-hover">
                <Nav.Link as={Link} to="/backoffice" state={{activeTab: 8}}>
                  Go Back
                </Nav.Link>
              </Nav.Item>
            </Button>
