...
const EditSection = () => {
  ...
  let [type, setType] = useState(state.type);
  let [isDisplayed, setIsDisplayed] = useState(state.isDisplayed);
  ...
  const saveSection = (title: string, content: string, type: string, isDisplayed: boolean) => {
    ...
    fetch(`api/section/${state.id}`, options)
      .then((response) => {
