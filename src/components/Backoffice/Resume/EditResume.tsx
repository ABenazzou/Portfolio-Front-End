...
const EditResume = () => {
  ...
  let [PDF, setPDF] = useState(state.PDF);
  let [lastUpdated, setLastUpdated] = useState(state.lastUpdated);
  // let [domainId, setDomainId] = useState(state.domainId);
  ...
  const saveResume = (PDF: string, lastUpdated: string) => {
    ...
    fetch(`api/resume/${state.id}`, options)
      .then((response) => {
