...
const EditProject = () => {
  ...
  let [github_link, setGithubLink] = useState(state.github_link);
  ...
  const handleSubmit = (event: {
    ...
    if (form.checkValidity() === false) {
      ...
      saveProject(name, thumbnail, github_link);
    }

    setValidated(true);
  };

  const saveProject = (name: string, thumbnail: string, github_link: string) => {
    ...
    fetch(`api/project/${state.id}`, options)
      .then((response) => {
        ...
        <Row xs={1} className="mb-3 mt-2">
          <Col className="mb-3">
            ...
            <Form noValidate validated={validated} onSubmit={handleSubmit}>
              ...
              <Form.Group className="d-flex mb-3">
                <FloatingLabel label="GithubLink">
                  <Form.Control
                    size="lg"
                    required
                    type="text"
