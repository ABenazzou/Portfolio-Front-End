...
const EditBiography = () => {
  ...
  let [startYear, setStartYear] = useState(state.startYear);
  let [endYear, setEndYear] = useState(state.endYear);
  let [occupation, setOccupation] = useState(state.occupation);
  ...
  const handleSubmit = (event: {
    ...
    if (form.checkValidity() === false) {
      ...
      savebiography(parseInt(startYear), parseInt(endYear), occupation);
    }
  ...
  const savebiography = (startYear: number, endYear: number, occupation: string) => {
    ...
    fetch(`api/biography/${state.id}`, options)
      .then((response) => {
